...
function getNextFriday(date) {
  const dateObj = new Date(date);
  if (dateObj.getDay() === 5) {
    dateObj.setDate(dateObj.getDate() + ((12 - dateObj.getDay()) % 14));
  } else {
...
function formatDate(date) {
  ...
  let minuits = dateObj.getMinutes();
  ...
  if (String(minuits).length === 1) {
    minuits = `0${minuits}`;
  }
  if (String(seconds).length === 1) {
    seconds = `0${seconds}`;
  }
  return `${month}/${day}/${year}, ${hours}:${minuits}:${seconds} ${dayPart}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstMonday = new Date(
    date.getFullYear(),
    0,
    januaryFirst.getDate() + daysToFirstMonday
  );
  if (daysToFirstMonday === 0 && date.getDay() !== 1) {
    return Math.ceil((date - firstMonday) / (24 * 3600 * 1000) / 7);
  }
  if (date.getDay() === 1) {
    return Math.ceil((date - firstMonday) / (24 * 3600 * 1000) / 7) + 2;
  }
  return Math.ceil((date - firstMonday) / (24 * 3600 * 1000) / 7) + 1;
}
